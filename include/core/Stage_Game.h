...
namespace Core {
    class Stage_Game : Stage {
        ...
        Stage_Game(
            std::vector<std::shared_ptr<Logic::Model::Entity>> entity_model,
            std::shared_ptr<Logic::Tile_Grid> grid,
            std::shared_ptr<Logic::Collision::World_Collision_Manager> WCM,
            std::vector<std::shared_ptr<Graphics::View::Entity_View>> entity_view,
            std::shared_ptr<Graphics::Camera> game_camera
            );
        ~Stage_Game() override;
