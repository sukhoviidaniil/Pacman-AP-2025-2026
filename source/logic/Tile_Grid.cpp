...
namespace Logic {
    ...
    rows_(rows), columns_(columns), tile_size_(tile_size){
        ...
        tiles = std::vector<std::vector<std::shared_ptr<Tile>>>(columns_, std::vector<std::shared_ptr<Tile>>(rows_));
        for (size_t row = 0; row < rows_; row++) {
            for (size_t column = 0; column < columns_; column++) {
                ...
                tiles[row][column] = std::make_shared<Tile>(hb);
            }
        }
    ...
    void Tile_Grid::update_Entity_Tile(const std::shared_ptr<Model::Entity> &entity) {
        ...
        std::shared_ptr<Tile> newTile = tiles[static_cast<size_t>(row)][static_cast<size_t>(col)];
