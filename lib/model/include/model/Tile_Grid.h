...
namespace model {
    ...
    public:
        ...
        std::shared_ptr<const Tile> get_tile(const size_t& x, const size_t& y) const;
        std::optional<math::Point2> get_tile_center(const size_t& x, const size_t& y) const;
        std::shared_ptr<const collision::HitBoxe> get_tile_hitboxe(const size_t& x, const size_t& y) const;
         ...
         */
        std::shared_ptr<const Tile> get_nearest_tile(const math::Point2 &pos) const;

        [[nodiscard]] std::shared_ptr<const Tile> get_next_tile(const math::Point2& pos, const math::Vector2& dir) const;
        [[nodiscard]] std::optional<math::Point2> get_next_center(const math::Point2 &pos, const math::Vector2& dir) const;

        [[nodiscard]] std::vector<std::vector<std::shared_ptr<const Tile>>> get_tiles() const;
